...
class SearchStationBloc extends Bloc<SearchStationEvent, SearchStationState> {
  ...
  Future<void> _onFetchStations(FetchStations event, Emitter<SearchStationState> emit) async {

    emit(state.copyWith(
        status: SearchStationStatus.loading
    ));
    ...
    try {
      ...
      if (responseData is Map) {
        ...
        rawList = (responseData['data'] as List<dynamic>?) ?? (responseData['items'] as List<dynamic>?) ??
            (responseData['stations'] as List<dynamic>?) ?? <dynamic>[];
      ...
      final List<StationInfo> stations = rawList.whereType<Map<String, dynamic>>().map(StationInfo.fromJson).toList();
