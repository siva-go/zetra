...
class WalletRepository {
  ...
  Future<Result<WalletModel>> fetchWallet() async {
    ...
    try {
      ...
      final WalletModel model = WalletModel.fromJson(data);
      return Result<WalletModel>.success(model);

    } on Object catch (e) {

      debugPrint('DEBUG FETCH WALLET ERROR: $e');
  ...
  Future<Result<WalletTransactionsPage>> fetchTransactions({int page = 1, int pageSize = 20}) async {
  ...
  Future<Result<TopupResponseModel>> initiateTopup({required int amountPaise, required String idempotencyKey}) async {
